---
name: colophon
description: Declares AI co-authorship with the Colophon sigil — a link at the end of comments, tickets, PR descriptions, issues, e-mails and chat messages, for Jira, Slack, GitHub and Trello, plus an HTML anchor for mail and wiki pages and a bare URL for callers that build the link themselves. Read this skill before marking any co-written text — the link is generated by a script, never reconstructed by hand. Applies whenever the AI contributed substantially, even without an explicit request; not for git commits (those use the Assisted-by trailer).
---
...
### The output is handed on, never reconstructed

Paste the stdout line verbatim where it is the finished sigil, or hand it on as a value — command substitution, a pipe, a variable.
Never rebuild the line from memory, and never re-type it into generated code as a string literal: a reconstructed line is where the character gets lost or the encoding surfaces as the visible label.
A script that posts obtains the line at run time — calling `sigil.py`, or importing its functions from Python — instead of carrying the finished line in its source.
Where the posting environment cannot reach `sigil.py`, pass the line in as data: an argument or environment variable filled from the script's output when the job is set up.
