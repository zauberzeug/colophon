---
name: colophon
description: Declares AI co-authorship at the end of comments, tickets, PR descriptions, issues, e-mails and chat messages. Read this skill before marking any co-written text — the mark is generated by a script, never written by hand. Applies whenever the AI contributed substantially, even without an explicit request; not for git commits (those use the Assisted-by trailer).
---
...
Paste the stdout line verbatim, or hand it on as a value — command substitution, a pipe, a variable.
...
A script that posts calls `sigil.py` itself at runtime instead of carrying the finished line in its source.
