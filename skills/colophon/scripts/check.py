...
through ``sigil.format_link`` for every target (and tooltip variant), escaped,
and the sentinel parts replaced by subpatterns. One shape source — a target
added to ``sigil.TARGETS`` is guarded automatically, and the patterns cannot
drift from what ``sigil.py`` actually emits.
...
def _tail_patterns():
    """One anchored pattern per link shape sigil.py emits — derived, not transcribed."""
    variants = [(target, None) for target in sigil.TARGETS]
    ...
    for target, tooltip in variants:
        ...
        if SIGIL not in template:
            continue  # `url` emits no label that could be damaged
